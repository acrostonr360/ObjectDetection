...
ap.add_argument("-d", "--detectionMode", default="",
	help="the method of detection we want to apply, for now onyl default")
...
ap.add_argument("-t", "--treshold", default=0.8,
	help="the threshold above wich we accept a detection as valid, between 0..1)")
ap.add_argument("-r", "--record", default="YES",
...
output = args["output"]+"/"+args["camera"]+"_SemanticLabel"
outputVideo = args["output"]+"/"+args["camera"]+"_SemanticLabel/"+args["camera"]+"_SemanticLabel.avi"	
if args["detectionMode"]=="sortLabels":
	if not os.path.isdir(output+"/HumanROI"):
		os.makedirs(output+"/HumanROI")	
	print("[INFO] Create dir:", output+"/HumanROI")
	if not os.path.isdir(output+"/chairROI"):
		os.makedirs(output+"/chairROI")	
	print("[INFO] Create dir:", output+"/chairROI")
	if not os.path.isdir(output+"/tableROI"):
		os.makedirs(output+"/tableROI")	
	print("[INFO] Create dir:", output+"/tableROI")
	if not os.path.isdir(output+"/plantROI"):
		os.makedirs(output+"/plantROI")	
...
#			leftFrame = cv2.cvtColor(b, cv2.COLOR_BayerBG2RGB)
		...
		if score >= treshold:
			...
			if args["detectionMode"]=="sortLabels":
				if classID==1:
					...
					roi = frameResize[startY:endY, startX:endX]
					#resize the ROI to normal image size
					#roi = imutils.resize(roi, (endX-startX)*2)
					roi = cv2.cvtColor(roi, cv2.COLOR_RGB2BGR)					
					cv2.imwrite(output+"/HumanROI/"+args["camera"]+"_"+format(nbFrame, '0>5')+"-"+str(humanCpt)+".png", roi)
					humanCpt+=1
				if classID==57:
					(startY, startX, endY, endX) = r["rois"][i]
					roi = frameResize[startY:endY, startX:endX]
					#roi = imutils.resize(roi, (endX-startX)*2)
					roi = cv2.cvtColor(roi, cv2.COLOR_RGB2BGR)
					cv2.imwrite(output+"/ChairROI/"+args["camera"]+"_"+format(nbFrame, '0>5')+"-"+str(chairCpt)+".png", roi)
					chairCpt+=1
				if classID==59:
					(startY, startX, endY, endX) = r["rois"][i]
					roi = frameResize[startY:endY, startX:endX]
					#roi = imutils.resize(roi, (endX-startX)*2)
					roi = cv2.cvtColor(roi, cv2.COLOR_RGB2BGR)
					cv2.imwrite(output+"/PlantROI/"+args["camera"]+"_"+format(nbFrame, '0>5')+"-"+str(plantCpt)+".png", roi)
					plantCpt+=1
				if classID==61:
					(startY, startX, endY, endX) = r["rois"][i]
					roi = frameResize[startY:endY, startX:endX]
					#roi = imutils.resize(roi, (endX-startX)*2)
					roi = cv2.cvtColor(roi, cv2.COLOR_RGB2BGR)
					cv2.imwrite(output+"/TableROI/"+args["camera"]+"_"+format(nbFrame, '0>5')+"-"+str(tableCpt)+".png", roi)
					tableCpt+=1
				if classID==1 or classID==57 or classID==59 or classID==61:
					frameResize = visualize.apply_mask(frameResize, mask, color, alpha=0.5)
	...
	if args["videoType"]=="stereo" or args["videoType"]=="flipStereo" or args["videoType"]=="lightSensorStereo":	
		if isLastRight:
			print("[INFO] Save Image:", outputStereoLeft+"/"+args["camera"]+"_Left_"+format(nbFrame, '0>5')+".png")
			cv2.imwrite(outputStereoLeft+"/"+args["camera"]+"_Left_"+format(nbFrame, '0>5')+".png", frame)
			isLastRight=0
		else:
			print("[INFO] Save Image:", outputStereoRight+"/"+args["camera"]+"_Right_"+format(nbFrame, '0>5')+".png")
			cv2.imwrite(outputStereoRight+"/"+args["camera"]+"_Right_"+format(nbFrame, '0>5')+".png", frame)
			isLastRight=1
	else:
		print("[INFO] Save Image:", output+"/"+args["camera"]+"_"+format(nbFrame, '0>5')+".png")
